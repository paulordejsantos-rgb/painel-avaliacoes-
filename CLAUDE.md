...
- Authentication (feito manualmente pelo Paulo em 19/09/2026): cadastro público desligado; proteção contra senhas vazadas ligada e salva no painel. O advisor de segurança do Supabase continua listando `auth_leaked_password_protection` ("Disabled") — é o único aviso restante e é FALSO POSITIVO: em 19/09/2026 o Paulo testou criar um usuário com senha vazada e o Supabase recusou, confirmando que a proteção está ativa (o advisor foi reverificado 5 vezes e não atualizou). Não investigar de novo, a menos que esse teste passe a aceitar senha vazada. O cadastro público desligado não é visível pelas ferramentas de verificação — só no painel.
